...
export const generateSpec = schemaTask({
  ...
  run: async (payload) => {
    // gemini-2.5-flash is not available to new-project Google AI accounts on
    // this project (see design-agent.ts / progress-tracker.md 23-design-agent-logic) —
    // matching design-agent.ts's already-confirmed-working model here too.
    ...
    // Human-readable filename for the UI (spec list display) — the Blob URL
    // alone (path-based, includes the project id and a raw timestamp) isn't
    // something a person should have to read to identify a spec.
    const filename = `system-spec-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, "-")}.md`
